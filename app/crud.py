...
from . import models, sschemas
from faastapi import HTTPException, status
import logging
...
def create_student(db: Session, student_in: schemas.StudentCreate):
    ...
    existing = db.query(models.Student).filter(models.Student.email == student_i.email).first()
    if existing:
